...
function logVerboseInfo(name, options) {
  ...
  console.log(`🎨 Estilo: ${options.style ? `✓ (.${options.styleExt})` : '✗ (none)'}`);
  console.log(`🧪 Teste: ${!options.noTest ? '✓ (.spec.js)' : '✗ (none)'}`);
...
function handleSuccess(name, result, options) {
  ...
  if (options.verbose) {
    ...
    console.log(`   import ${componentName} from './${fileName}/${fileName}.vue'`);
  }
}
