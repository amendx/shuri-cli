...
const { vue2Template, vue3Template, testTemplate, styleTemplate, indexTemplate } = require('../helpers/templates');
...
function resolveComponentNames(name, options) {
  ...
  return { pascal, kebabName, fileName };
}

function generateTemplateContent(names, vueVersion, files) {
  const { pascal, kebabName, fileName } = names;
  
  return {
    component: vueVersion === 3
      ? vue3Template(pascal, files.style)
      : vue2Template(pascal, files.style),
    style: styleTemplate(pascal, kebabName),
    test: testTemplate(pascal, vueVersion),
    index: indexTemplate(pascal, fileName)
  };
}
...
async function createComponent(name, options = {}) {
  ...
  const componentDir = path.join(outBase, names.fileName);
